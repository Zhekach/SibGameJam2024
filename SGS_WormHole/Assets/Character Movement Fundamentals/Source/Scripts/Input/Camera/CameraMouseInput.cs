...
namespace CMF
{
    ...
    public class CameraMouseInput : CameraInput
    {
        ...
        public void RefreshMouseSettings()
        {
            mouseInputMultiplier = PlayerPrefs.GetFloat(MouseSettings.MouseSense.ToString(), 0.01f);

            invertHorizontalInput = Convert.ToBoolean(PlayerPrefs.GetInt(MouseSettings.MouseInvertX.ToString()));
            invertVerticalInput = Convert.ToBoolean(PlayerPrefs.GetInt(MouseSettings.MouseInvertY.ToString()));
        }
